...
module.exports = function GitHubCalendar (container, username, options) {
    ...
    let fetchCalendar = () => options.getCalendar(username).then(body => {
        ...
        // Remove "Skip to contributions year list" link only
        for (const a of div.querySelectorAll("a")) {
            if (a.textContent.includes("Skip to contributions year list")) {
                a.remove()
            }
        }

        // Remove "Learn how we count contributions" link
        for (const a of div.querySelectorAll("a")) {
            if (a.textContent.includes("Learn how we count contributions")) {
                a.remove()
            }
        }
        ...
        // Remove footer and stats elements
        for (const selector of ['.contrib-column', '.contrib-number', '.text-small', '.contrib-legend', '.contrib-footer', '#user-activity-overview', '.calendar .width-full > .float-left']) {
